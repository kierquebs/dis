...
class Account extends MX_Controller {
	...
	public function index(){		
		...
		$data['js'][] = 'account.js';			
		$this->lib_order->layout_nav('account/index', $data);
	}
	...
	public function password(){
		...
		$this->form_validation->set_rules('passnew', 'New Password', 'required|min_length[8]');	
		...
		if ($this->form_validation->run() !== FALSE){
			/* save to db */
			$data['alert'] = $this->lib_order->alertMsg(7, 'Successfully Change Password', true);
		...
		$data['css'][] = 'account.css';		
		$data['js'][] = 'account.js';			
		$this->lib_order->layout_nav('account/password', $data);
	}
		/**
		 * Check Password 
		 * @param  var  $str = passold
