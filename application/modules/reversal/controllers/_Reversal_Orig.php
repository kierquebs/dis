				...
				if(is_array($result_data) && COUNT($result_data) != 0){	
					...
					if(!empty($get_UploadID)){
						for($i=0;$i<COUNT($result_data);$i++){
							...
							if($get_TempID <> false){
								$check_Refund = $this->Sys_model->v_refund(array('REDEEM_ID'=>$arrData['REDEMPTION_API_TRANSACTION_ID'], 'PROD_ID'=>$arrData['PROD_ID']), TRUE);
