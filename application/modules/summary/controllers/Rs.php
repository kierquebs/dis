...
class Rs extends MX_Controller {
	...
	public function index(){ 
		//echo computeExpectedDueDate($dayType, $qtyOfDays, $paGenDate)
		$data['css'][] = 'queue.css';
		$data['js'][] = 'main.js';
		$data['js'][] = 'home.js';
		$data['js'][] = 'summary/rs_main.js';			
				
		$data['cssDT'][] = '1.10.16/css/jquery.dataTables.min.css';
		$data['jsDT'][] = '1.10.16/js/jquery.dataTables.min.js';	
		$this->my_layout->layout_nav('summary/rs', $data);
	}
/*
* ajax form request
*/	
	...
	public function get_item(){
		...
		if(isset($_GET['search'])  && $_GET['search'] != '' ){
			...
			$where .= (empty($where) ? '' : ' AND rsH.RS_ID in ('.$this->my_lib->multiPANUM($SEARCH).')'); 
		}
		if(isset($_GET['mid'])  && $_GET['mid'] != '' ){
			$SEARCH = htmlentities($this->input->get('mid', true));
			$where .= (empty($where) ? '' : ' AND (conv_tbl.MERCHANT_ID = "'.$SEARCH.'")');
		}
		if(isset($_GET['mname'])  && $_GET['mname'] != '' ){
			$SEARCH = $this->input->get('mname', true);
			$where .= ' AND (conv_tbl.LegalName like "%'.$SEARCH.'%")';
		}
		//*END FILTER SEARCH
		...
		$GROUP_BY =  'rsD.RS_ID';
		$data['total'] =  $this->Sys_model->getReimSummary($where, true, '', $GROUP_BY)->num_rows();
		if($data['total'] != 0){	
			$temp_transac =  $this->Sys_model->getReimSummary($where, false, $page,  $GROUP_BY);	
	...
	public function export(){	
		...
		//*END FILTER SEARCH
		$GROUP_BY =  'rsD.RS_ID';
		$result =  $this->Sys_model->getReimSummary($where, false, '',  $GROUP_BY);	
		if($result->num_rows() != 0 ){
			$arr = $this->arr_result($result);
			$this->load->model('Action_model');								
			$this->Action_model->audit_save(7, '');	
			$this->load->library('download_file');
			return $this->download_file->summary_report(array('filename'=>'REIMBURSEMENT SUMMARY'), $arr);
		}
	}
		...
		private function arr_result($temp_transac, $export = false){
			...
			foreach($temp_transac->result() as $temp_row){ 
				...
				foreach ($fields as $field){
					if($field == 'TOTAL_FV' && $temp_row->$field != '') $newRow->$field = number_format($temp_row->$field, 2);
