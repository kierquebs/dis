...
/**
 * This module will only process merchants tag as "NO RECON"
 */
class Nrecon extends MX_Controller {
	private $MODULE_ID;
	private $checkUpload;
	...
	public function index(){ 
		...
		//$where, $count, $select	
		$whereCut = "SPECIFIC_DATE <> '' AND DigitalSettlementType <> ''";	
		...
		$this->my_layout->layout_nav('process/nrecon_index', $data);				
	}
...
*/	
	public function get_item(){
		...
		if($this->checkUpload == 1){		
			...
			if($paymentTerms  != ''){		
				...
				if($data['terms'] == 3){		
					if(isset($_GET['day'])  && $_GET['day'] != '' ){
						...
						$where .= $where_date = ' AND DATE_FORMAT(redeem.TRANSACTION_DATE_TIME, "%Y-%m-%d") <= "'.$dateWhere.'"'; 
					}
				}else{
			...
			$data['total'] =  $this->Process_model->getPCFNRecon($where, true, '', $dateWhere, $pcfWHere); //getPaymentCutoff
			if($data['total'] != 0){	
				$temp_transac =  $this->Process_model->getPCFNRecon($where, false, $page,$dateWhere, $pcfWHere);	//getPaymentCutoff	
			...
			foreach($temp_transac as $temp_row){ 
				$newRow = new stdClass(); 						
					...
					//** check UNKNOWN TRANSACTIONS
						...
						$where .= $where_date;
						$unkownTransac = $this->Process_model->getPCFNRecon_Branch($where, false);
					...
					//** check VALID TRANSACTIONS
						...
						$where2 .= $where_date;
						$newRow->validTransac = $this->Process_model->getPCFNRecon_Branch($where2, true);
						
					
					//** get refund records	
					$newRow->refundAmount = $temp_row->refundAmount; 
					$newRow->totalRefTransac = $temp_row->totalPassesRef; 
	...
	public function get_branches(){
		...
		if($_POST['terms'] == 3){		
			if(isset($_POST['day'])  && $_POST['day'] != '' ){ 
				...
				$where .= $where_date = ' AND DATE_FORMAT(redeem.TRANSACTION_DATE_TIME, "%Y-%m-%d") <= "'.$this->my_lib->setCFDay($day).'"'; //$this->my_lib->setCFDate($_GET['date'])
			}
		}else{
			if(isset($_POST['date'])  && $_POST['date'] != '' ){
				...
				$where .= $where_date = ' AND DATE_FORMAT(redeem.TRANSACTION_DATE_TIME, "%Y-%m-%d") <= "'.$this->my_lib->setCFDate($date).'"'; //$this->my_lib->current_date()
			}	
		}
		$data['where'] = $where;	
		$page = $this->my_layout->pagination();	
		$data['per_page'] = $page['per_page'];	
		$data['offset'] = $page['offset'];	
		$data['total'] =  $this->Process_model->getPCFNRecon_Branch($where, true);
		if($data['total'] != 0){	
			$temp_transac =  $this->Process_model->getPCFNRecon_Branch($where, false, $page);			
