...
/**
 * This module will only process merchants "with reconciliation transaction"
 */
class Wrecon extends MX_Controller {
	private $MODULE_ID;
	private $checkUpload;
	...
	public function index(){ 
		...
		$this->my_layout->layout_nav('process/wrecon_index', $data);				
	}
...
*/	
	public function get_item(){
		...
		if($this->checkUpload == 1){		
			...
			$data['total'] =  $this->Process_model->getPCFWRecon($where, true, '', $dateWhere, $pcfWHere); //getPaymentCutoff
			if($data['total'] != 0){	
				$temp_transac =  $this->Process_model->getPCFWRecon($where, false, $page,$dateWhere, $pcfWHere);	//getPaymentCutoff	
			...
			foreach($temp_transac as $temp_row){ 
				$newRow = new stdClass(); 						
					...
					//** check UNKNOWN TRANSACTIONS
						...
						$where .= $where_date;
						$unkownTransac = $this->Process_model->getPCFWRecon_Branch($where, false);
					...
					//** check VALID TRANSACTIONS
						...
						$where2 .= $where_date;
						$newRow->validTransac = $this->Process_model->getPCFWRecon_Branch($where2, true);
	...
	public function get_branches(){
		...
		$data['per_page'] = $page['per_page'];	
		$data['offset'] = $page['offset'];	
		$data['total'] =  $this->Process_model->getPCFWRecon_Branch($where, true);
		if($data['total'] != 0){	
			$temp_transac =  $this->Process_model->getPCFWRecon_Branch($where, false, $page);			
