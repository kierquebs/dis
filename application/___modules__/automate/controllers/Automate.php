...
class Automate extends MX_Controller {
		...
		for($x=0; $x<=count($map); $x++){
			...
			if(is_array($result_data)){	
				...
				for($i=0;$i<count($result_data);$i++){
					$insertDB['BRANCH_ID'] = $where['BRANCH_ID'] = $result_data[$i]['BRANCH_ID'];
					$updateCP['MERCHANT_ID'] = $insertDB['MERCHANT_ID'] = $where['MERCHANT_ID'] = $result_data[$i]['MERCHANT_ID'];										
					$insertDB['BRANCH_NAME'] = $result_data[$i]['BRANCH_NAME'];
					
					$recordResult = $this->Sys_model->v_branches($where, true);
					if($recordResult == 0) $this->Sys_model->i_branches($insertDB);
					else $this->Sys_model->u_branches($where['BRANCH_ID'], $insertDB);
	 ...
	 * GET ALL DATA FROM CSV FILE
	 */
	public function upload_fee(){
		$map = directory_map('./to_upload/merchant_fee/', FALSE, TRUE);
		for($x=0; $x<=count($map); $x++){
			$fileArr = array();	$return = false;
			$fileArr['filname']  = $map[$x++];
			$fileArr['module']  = 'merchant_fee'; 		
			$this->load->library('upload_file');
			...
			if(is_array($result_data)){	
				...
				echo "SUCCESS INSERT MERCHANT FEE : <br /> <pre>";
				for($i=0;$i<count($result_data);$i++){
					$where['MERCHANT_ID'] = $result_data[$i]['MERCHANT_ID'];
					
					$recordResult = $this->Sys_model->v_mfee($where, true);
					if($recordResult == 0) $this->Sys_model->i_mfee($result_data[$i]);
					else $this->Sys_model->u_mfee($where['MERCHANT_ID'], $result_data[$i]);
