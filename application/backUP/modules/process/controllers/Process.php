		...
		if($countProcess <> 0){			
			...
			if($_POST['terms'] == 3){		
				if(isset($_POST['day'])  && $_POST['day'] != '' ){
					...
					$where .= ' AND DATE_FORMAT(recon.RECON_DATE_TIME, "%Y-%m-%d") <= "'.$this->my_lib->setCFDay($day).'"'; 
				}
			}else{
				if(isset($_POST['date'])  && $_POST['date'] != '' ){
					$date = htmlentities($this->input->post('date', true));
					$where .= ' AND pcf.SPECIFIC_DATE like "%'.$date.'%"'; 
					$where .= ' AND DATE_FORMAT(recon.RECON_DATE_TIME, "%Y-%m-%d") <= "'.$this->my_lib->setCFDate($date).'"';
				}	
			}
			...
			for($i=0; $i<=$countProcess; $i++){
				if(!empty($toProcess[$i])){
					...
					$getPCBranchesRow =  $this->Sys_model->getPCBranch_PA($whereBranch, false);	 //getPaymentCutoffBranch
						...
						if(!empty($PA_ID)){		
							...
							foreach($getPCBranchesRow->result() as $row){
								...
								*/
								...
								$this->Sys_model->u_recon($whereBranch, $uRECON);
							}
							$u_paH['MERCHANT_FEE']= $PA_MerchantFee;
							$u_paH['vatcond']= $PA_VAT;
							$u_paH['ExpectedDueDate']= $this->my_lib->computeExpectedDueDate($PA_PayeeDayType, $PA_PayeeQtyOfDays, $insert_header['REIMBURSEMENT_DATE']);
