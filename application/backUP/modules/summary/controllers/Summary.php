...
*/	
	...
	public function get_item(){
		...
		if(isset($_GET['mid'])  && $_GET['mid'] != '' ){
			...
			$where .= (empty($where) ? '' : ' AND (recon_tbl.MERCHANT_ID = "'.$SEARCH.'")');
		}
		if(isset($_GET['mname'])  && $_GET['mname'] != '' ){
			$SEARCH = $this->input->get('mname', true);
			$where .= ' AND (recon_tbl.LegalName like "%'.$SEARCH.'%")';
		}
		//*END FILTER SEARCH
		...
		$GROUP_BY =  'paD.PA_ID';
		$data['total'] =  $this->Sys_model->getTransactionSummary($where, true, '', $GROUP_BY)->num_rows();
		if($data['total'] != 0){	
			$temp_transac =  $this->Sys_model->getTransactionSummary($where, false, $page,  $GROUP_BY);	
	...
	public function export(){	
		...
		//*END FILTER SEARCH
		$GROUP_BY =  'paD.PA_ID';
		$result =  $this->Sys_model->getTransactionSummary($where, false, '',  $GROUP_BY);	
