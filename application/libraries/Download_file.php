...
class Download_file extends MX_Controller{
	...
	public function load_file($module, $query){
		...
		$spreadsheet = $this->_callPHPCLASS($module);

		// Field names in the first row — PhpSpreadsheet columns are 1-based
		...
		$this->_callDownload($spreadsheet, $module);
		return true;
	}
 ...
 */
	...
	private function _callPHPCLASS($module){
        ...
        $spreadsheet->getProperties()->setTitle($module['filename'])->setDescription("");
		$spreadsheet->setActiveSheetIndex(0);
		return $spreadsheet;
	}
	/**
	 ...
	 *
	 * @param Spreadsheet $arrOBJ
	 * @param array $module
	 * @param string $type {'Xls' , 'CSV'}
	 * @return void
	 */
	private function _callDownload($arrOBJ, $module, $type = 'Xls'){
		...
		if($type == 'CSV'){
			$objWriter = new CsvWriter($arrOBJ);
			$objWriter->setUseBOM(true);
			$objWriter->setEnclosure('');
		}else{
			$objWriter = new XlsWriter($arrOBJ);
		}

		return $objWriter->save('php://output');
	}

	private function _callDownloadNew($arrOBJ, $module, $type = 'Xls'){
		$filename = $module['filename'];
		header('Content-Type: application/force-download;');
		header('Content-Transfer-Encoding: binary'); //no cache
		header('Cache-Control: max-age=0'); //no cache
	...
	private function _callDownloadServer($arrOBJ, $module, $type = 'Xls', $serverDL = ''){
 ...
 */
	...
	public function transaction_report($module, $result){
		...
		foreach($result as $row){
			...
			$spreadsheet->getActiveSheet()->getRowDimension($x)->setRowHeight(-1);
			$spreadsheet->getActiveSheet()->getStyle("A$x:S$x")->getAlignment()->setWrapText(true);
			$x++;
		}
	...
	public function summary_report($module, $result){
		...
		foreach($result as $row){
			...
			$spreadsheet->getActiveSheet()->getRowDimension($x)->setRowHeight(-1);
			$spreadsheet->getActiveSheet()->getStyle("A$x:E$x")->getAlignment()->setWrapText(true);
			$x++;
		}
	...
	public function _nav_merchant($module, $result, $serverDl = true){
		...
		foreach($result as $row){
			...
			$spreadsheet->getActiveSheet()->getRowDimension($x)->setRowHeight(-1);
			$spreadsheet->getActiveSheet()->getStyle("A$x:R$x")->getAlignment()->setWrapText(true);
			$x++;
		}
		...
		if($serverDl == true) $this->_callDownloadServer($spreadsheet, $module, 'CSV', $this->navLocation);
		else $this->_callDownload($spreadsheet, $module, 'CSV');
	}
	...
	public function _nav_remittance($module, $result, $serverDl = true){
		...
		foreach($result as $row){
			...
			$spreadsheet->getActiveSheet()->getRowDimension($x)->setRowHeight(-1);
			$spreadsheet->getActiveSheet()->getStyle("A$x:T$x")->getAlignment()->setWrapText(true);
			...
			if(!empty($navDetail)){
				...
				foreach($navDetail as $detail){ //FACE VALUE
					...
					$spreadsheet->getActiveSheet()->getRowDimension($y)->setRowHeight(-1);
					$spreadsheet->getActiveSheet()->getStyle("A$y:D$y")->getAlignment()->setWrapText(true);
				}
				$y += 1;
				$spreadsheet->setActiveSheetIndex(0);
				$spreadsheet->getActiveSheet()
				...
				$spreadsheet->getActiveSheet()->getRowDimension($y)->setRowHeight(-1);
				$spreadsheet->getActiveSheet()->getStyle("A$y:F$y")->getAlignment()->setWrapText(true);
	...
	public function _cic_si_remittance($module, $result, $serverDl = true){
		...
		foreach($newArr as $row){
			...
			$spreadsheet->getActiveSheet()->getRowDimension($x)->setRowHeight(-1);
			$spreadsheet->getActiveSheet()->getStyle("A$x:Q$x")->getAlignment()->setWrapText(true);
		...
		$module['filename'] = $module['filename'].'.csv';
		if($serverDl == true) $this->_callDownloadServer($spreadsheet, $module, 'CSV', $this->navLocation);
		else $this->_callDownloadNew($spreadsheet, $module, 'CSV');
	}
			...
			if(!empty($navDetail)){
				...
				foreach($navDetail as $key => $detail){ //FACE VALUE
					...
					if (strpos($billableItem3, "Fee") === false) {
						...
						$spreadsheet->getActiveSheet()->getRowDimension($y)->setRowHeight(-1);
						$spreadsheet->getActiveSheet()->getStyle("A$y:H$y")->getAlignment()->setWrapText(true);
					}
				}
				$x = $y;
			}
	...
	public function rs_txtfile($module, $result, $serverDl = true, $servLoc = ''){
		...
		if($serverDl == true && !empty($servLoc)) $this->_callDownloadServer($spreadsheet, $module, 'CSV', $servLoc);
		else $this->_callDownload($spreadsheet, $module, 'CSV');
	}
	 ...
	 public function _dormancy_fee($module, $result, $serverDl = true){
		...
		foreach($result as $row){
			...
			$spreadsheet->getActiveSheet()
				...
				->setCellValue("O$x",$row->CP_ID);
			$spreadsheet->getActiveSheet()->getRowDimension($x)->setRowHeight(-1);
			$spreadsheet->getActiveSheet()->getStyle("A$x:O$x")->getAlignment()->setWrapText(true);
