...
class My_layout extends MX_Controller{
    ...
    public function layout_nav($page, $data = ''){
		...
		private function set_nav(){
			...
			switch ($module_class) {
				...
				case 'rs':
					if($page_active == 3){
						$page_title = 'REIMBURSEMENT SUMMARY';
						$page_active = 6;
					}else{
						$page_title = 'REIMBURSEMENT SUMMARY PROCESS';
						$page_active = 4;
					}
				break;
			}	
	...
	*/
	...
	/**
     * USER SET PERMISSION per MODULE
	 * access role permission:
		1	transaction	transaction
		2	process	process
		3	summary	summary
		4	rs_process	rs_process
		5	admin	admin
		6	rs_summary	rs_summary
    ...
    public function user_permission($typeID, $moduleID){
		...
		switch ($typeID) {
			...
			break;
			case 4:	//FINANCE	
				if($moduleID == 5 || $moduleID == 2 && $moduleID == 4){
					$arr['acc_read_only'] = 0;
					$arr['acc_read_only'] = $arr['acc_all_access'] = $arr['def_page'] = 0;
				}else{
					$arr['acc_read_only'] = 1;
