...
*/	
	...
	public function get_item(){
		...
		if(isset($_GET['mid'])  && $_GET['mid'] != '' ){
			...
			$where .= (empty($where) ? '' : ' AND (br.MERCHANT_ID = "'.$SEARCH.'")');
		}
		...
		$GROUP_BY =  'paH.PA_ID';
		$data['total'] =  $this->Sys_model->getTransaction($where, true, '', $GROUP_BY)->num_rows();
		if($data['total'] != 0){	
			$temp_transac =  $this->Sys_model->getTransaction($where, false, $page,  $GROUP_BY);	
	...
	public function export(){	
		...
		//*END FILTER SEARCH
		$GROUP_BY =  'paH.PA_ID';
		$result =  $this->Sys_model->getTransaction($where, false, '',  $GROUP_BY);	
