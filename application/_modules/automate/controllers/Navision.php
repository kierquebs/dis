	 ...
	 */
	 ...
	 private function _interface_merchant($serverDl = true){
		...
		if($result->num_rows() != 0 ){
			...
			$module['filename'] = 'DMC_'.$this->my_lib->setDate('', TRUE).'_01'; //DMC_MMDDYYYY_01.csv
	 ...
	 */	 
	 private function _interface_remittance($serverDl = true){
		...
		if($result->num_rows() != 0 ){
			...
			$module['filename'] = 'DAR_'.$this->my_lib->setDate('', TRUE).'_01'; //DAR_MMDDYYYY_01.csv
		...
		private function _interface_remittance_result($temp_transac, $export = false){
			...
			foreach($temp_transac->result() as $temp_row){ 
				...
				navision detail
				*/
				$whereD = 'AND paH.PA_ID = '.$temp_row->paymentAdvice.' AND recon.PROD_ID = '.$temp_row->PROD_ID;	
