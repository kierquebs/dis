...
function PopulateItemsTable() {
	$.ajax({
	type: "POST",
	url: "replacement/get_act?page="+page+search,
	dataType: "json",
	success: function (data) {
		...
		if(data.totalData != 0){
			...
			$.each(data.result, function(key, value) {
				...
				$orderinfo = $('<span></span>')				
							.attr({'data-prod':$orderINFO['coprod'], 'data-reason':$orderINFO['coreason'], 'data-charge':$orderINFO['cocharge'], 'data-amount':$orderINFO['amount'], 'data-date':$orderINFO['date'], 'data-qnty':$orderINFO['coqnty'], 'data-amntchgr':$orderINFO['cochargeamt'], 'data-rsnum':$orderINFO['cors'], 'data-type':$orderINFO['cotype'], 'data-action':$orderINFO['coaction'], 'data-em':$orderINFO['coem'], 'data-expiry':$orderINFO['coexpiry'], 'data-cat':$orderINFO['cocatname']})				
							.addClass('orderinfo')
							.text('VIEW INFO');
				$date1 = $('<span></span>')
							.addClass('date')
				...
				*/
				...
				for (i = 6; i <= 7; i++) { 
					switch(i) {						
						...
						case 7:
							...
							if(value.cs_stat != 0 && value.reim_stat == 0) $valBTN = data.reim_btn;
							else $valBTN = false;
							break;
					}
					$orderTxt = 'SELECT';			
				...
				var newRow = myTable.row.add(result).draw().node(); 
					$(newRow).addClass('queue-tr'+($orderINFO['coaction_id'] == 3 ? ' yellow-tag' : ''))
							.attr('data-id', $TRANSID) 
								...
								.find('td:nth-child(6)').addClass('queue-tbl-span co-reim').append($reimBtn).append($reimTIME);
			});				
...
function viewInfo(){	
	...
	$('.orderinfo').click(function(e){	
		setModal(modalID2, 'ORDER REPLACEMENT/REVALIDATION: DETAILS', 'OK');
...
function orderModal(){
	...
	$('.td-btn button').click(function(e){	
		setModal(modalID, 'PROCESS ORDER REPLACEMENT/REVALIDATION', 'OK');	
		$me = $(e.currentTarget); 
		/* CHECK FORM TITLE */
		if($me.attr('data-name') == 'cs') $Title = 'CUSTOMER SERVICE';
